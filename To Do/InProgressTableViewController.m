...
@implementation InProgressTableViewController {
    NSMutableArray *inProgressToDoItems;
...
- (void)viewDidLoad {
    ...
    inProgressToDoItems = [NSMutableArray new];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    for (int index = 0; index < [toDoItems count]; index++) {
        ...
        if([currentItem.iStatus  isEqual: @"in progress"])
        {
            [inProgressToDoItems addObject:currentItem];
        }
...
#pragma mark - Table view data source
    ...
    - (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
        return [inProgressToDoItems count];
    }
     ...
     // Configure the cell...
         NSString *imageName;
             ...
             cell.textLabel.text = [[inProgressToDoItems objectAtIndex:indexPath.row] iName];
             cell.detailTextLabel.text = [[inProgressToDoItems objectAtIndex:indexPath.row] iName];
             cell.accessoryType = UITableViewCellAccessoryDetailDisclosureButton;
             
             if([[[inProgressToDoItems objectAtIndex:indexPath.row]iPriority]
                 isEqual: @"low"])
             {
                 imageName = @"low.png";
             } else if ([[[inProgressToDoItems objectAtIndex:indexPath.row]iPriority]
                         isEqual: @"high"]) {
                 imageName = @"high.png";
             }else if ([[[inProgressToDoItems objectAtIndex:indexPath.row]iPriority]
                        isEqual: @"medium"]) {
